...
import { ProviderResult, RiskAssessmentInput } from './types';
...
describe('AiAgentService', () => {
  ...
  it('calls provider.assess with correctly mapped input', async () => {
    ...
    const agent = new AiAgentService(mockProvider);
  ...
  it('result contains all required fields', async () => {
    const agent = new AiAgentService(buildMockProvider());
  ...
  it('calls logger.info for LOW risk level', async () => {
    const agent = new AiAgentService(buildMockProvider({ riskLevel: 'LOW' }));
    await agent.assess(mockRequest);
  ...
  it('calls logger.info for MEDIUM risk level', async () => {
    const agent = new AiAgentService(buildMockProvider({ riskLevel: 'MEDIUM' }));
    await agent.assess(mockRequest);
  ...
  it('calls logger.warn for HIGH risk level', async () => {
    const agent = new AiAgentService(buildMockProvider({ riskLevel: 'HIGH', score: 72 }));
    await agent.assess(mockRequest);
  ...
  it('calls logger.warn for CRITICAL risk level', async () => {
    const agent = new AiAgentService(buildMockProvider({ riskLevel: 'CRITICAL', score: 88 }));
    await agent.assess(mockRequest);
